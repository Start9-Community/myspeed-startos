...
This repo packages [MySpeed](https://github.com/gnmyt/myspeed) for StartOS.
...
The `main` image is built locally from `Dockerfile`, which pins upstream via the `FROM` line. There is no `dockerTag` in the manifest.

1. Bump the `FROM germannewsmaker/myspeed:<version>` line in `Dockerfile` to the new upstream tag.
